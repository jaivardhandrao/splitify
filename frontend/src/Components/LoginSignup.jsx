...
function LoginSignup() {
  ...
  const [notification, setNotification] = useState(''); // New: For success notifications

  const API_BASE = 'http://localhost:5666/api'; // Your backend port

  // New: Function for notification popup (similar to Dashboard)
  const showNotification = (msg) => {
    setNotification(msg);
    setTimeout(() => setNotification(''), 3000); // 3s duration for better visibility
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        showNotification('Login successful! Redirecting...'); // Replaced alert
        // Delay redirect slightly for notification to show
        setTimeout(() => {
        ...
        showNotification('Registration successful! Check your email for verification link.'); // Replaced alert
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      {/* New: Notification Popup (Green success toast, similar to Dashboard) */}
      {notification && (
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="bg-white relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span
              className={`${isLogin ? 'bg-white px-3 py-2 text-gray-900 font-semibold' : 'bg-transparent px-3 py-2 text-gray-500'
  ...
  @keyframes check { 0%, 100% { transform: scale(0); } 50% { transform: scale(1.1); } }
  .animate-check { animation: check 0.5s ease-in-out; }
`}</style>
    </div>
  );
}

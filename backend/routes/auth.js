...
router.post('/register', async (req, res) => {
  try {
    ...
    const msg = {
      to: email,
      from: 'noreply@yourdomain.com', // Replace with verified sender (e.g., your Gmail or custom domain)
      subject: `Welcome to Splitify! Verify Your Email to Get Started`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p style="font-size: 11px; color: #999; margin-top: 20px;">
            <strong>Unsubscribe:</strong>
            <a href="mailto:noreply@yourdomain.com?subject=Unsubscribe Splitify" style="color: #10b981;">click here</a>.
          </p>
          <div style="text-align: center; margin-top: 30px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 12px; color: #666;">
            © 2025 Splitify. All rights reserved. | Made with ❤️ for easy expense splitting.
          </div>
...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '7d' });
    res.json({ token, user: { id: user._id, email: user.email, name: user.name, phone: req.user.phone } });
  } catch (error) {
    console.error('Login error:', error);
    res.status(500).json({ error: 'Server error during login' });
  }
